...
direct_declarator:
                    ...
                    | direct_declarator OPENRB FUN_CT parameter_type_list CLOSERB 
                    {
                        string nameOfTable = $1->name;
                        if(nameOfTable.find(".")!=nameOfTable.npos)
                            nameOfTable = nameOfTable.substr(0, nameOfTable.find("."));
                    ...
                    | direct_declarator OPENRB FUN_CT CLOSERB 
                    {        //similar as above
...
selection_statement:
                    IF OPENRB expression CLOSERB M statement N %prec "then"
                    {
                        // if without else
                    ...
                    | IF OPENRB expression CLOSERB M statement N ELSE M statement
                    {
                        // if with else
...
function_definition:
                    declaration_specifiers declarator declaration_list_opt FUN_CT OPENCB block_item_list_opt CLOSECB 
                    {
                        // int next_instr=0;	 	
